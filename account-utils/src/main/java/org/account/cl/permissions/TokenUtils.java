...
public class TokenUtils {
    ...
    public static String generateToken(final String username, final long expire, final String key) {
        ExceptionEnum.INTERNAL_INVALID_PARAMETER.assertNotNull(username, "TokenUtils:generateToken", "username", username);
        ExceptionEnum.INTERNAL_INVALID_PARAMETER.assertNotNull(key, "TokenUtils:generateToken", "key", key);
        ExceptionEnum.INTERNAL_INVALID_PARAMETER.assertTrue(expire > 1, "TokenUtils:generateToken", "expire", expire);
    ...
    public static Claims getClaimByToken(String token, final String key) throws ExpiredJwtException {
        ExceptionEnum.INTERNAL_INVALID_PARAMETER.assertNotNull(token,"TokenUtils:getClaimByToken", "token", token);
        ExceptionEnum.INTERNAL_INVALID_PARAMETER.assertNotNull(key,"TokenUtils:getClaimByToken", "key", key);
